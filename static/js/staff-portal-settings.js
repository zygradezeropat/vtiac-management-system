/**
 * Shared staff portal settings — profile + password (server-backed).
 */
...
function wirePasswordToggles(root, toggleClass) {
 ...
 * @param {string} [options.passwordToggleClass] - CSS class on show/hide password buttons
 * @param {string} [options.displayNameId] - Element id to update after profile save
 * @param {string} [options.initialsId] - Optional element id for initials (trainer header)
 */
export async function initStaffPortalSettings(options = {}) {
  const passwordToggleClass = options.passwordToggleClass || "staff-password-toggle";
  const displayNameId = options.displayNameId || "staff-display-name";
  const initialsId = options.initialsId || "";
  ...
  wirePasswordToggles(document, passwordToggleClass);
  ...
  profileForm?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const nameEl = document.getElementById(displayNameId);
      const initialsEl = initialsId ? document.getElementById(initialsId) : null;
